...
const AdminPeople = {
  ...
  build: function(self, s, accent, ctx) {
    ...
    return {
      ...
      leaveSearchOpen:!!(s.leaveSearchOpen), toggleLeaveSearch:()=>self.setState({leaveSearchOpen:!s.leaveSearchOpen}),
      leaveTypeFilter:s.leaveTypeFilter||'all',
      ...
      ...(()=>{
        ...
        return {
          pendingLeaves:_sorted.map(l=>{const _ms=l.created_at?_nowMs-new Date(l.created_at).getTime():0,_d=Math.floor(_ms/86400000),isExpired=_ms>_2d,timeAgo=!l.created_at?'':Utils.hhmm(new Date(l.created_at)),_av=s.avatars[l.personnel_id]||'';return({
            ...
            avatarStyle:Utils.avatarStyle(_av),
            avatarInitials:_av?'':Utils.initials(l.personnel?.name||'?'),
            ...
            typeBg:l.type==='mc'?'#fdf6e9':'#fefce8',
            typeColor:l.type==='mc'?'#b9791a':'#7c5e00',
            typeBorder:l.type==='mc'?'#f0e2c2':'#fde68a',
      ...
      ...(()=>{
        const _plq=(s.processedLeavesSearch||'').toLowerCase().trim();
        const _plf=s.processedLeavesFilter||'all';
        const _PL_PS=10;
        let _pll=s.processedLeaves||[];
        if(_plq) _pll=_pll.filter(l=>(l.personnel?.name||'').toLowerCase().includes(_plq)||(l.personnel?.contact||'').includes(_plq));
        if(_plf!=='all') _pll=_pll.filter(l=>_plf==='mc'?l.type==='mc':(l.type==='personal'||l.type==='other'));
        const _plTotal=_pll.length,_plTotalPages=Math.max(1,Math.ceil(_plTotal/_PL_PS));
        const _plPage=Math.min(Math.max(1,s.processedLeavesPage||1),_plTotalPages);
        const _plPaged=_pll.slice((_plPage-1)*_PL_PS,_plPage*_PL_PS);
        ...
        return {
          processedLeavesItems:_plPaged.map(l=>{const _av=s.avatars[l.personnel_id]||'';return({
            id:l.id,
            personName:l.personnel?.name||'Unknown',
            avatarStyle:Utils.avatarStyle(_av),
            avatarInitials:_av?'':Utils.initials(l.personnel?.name||'?'),
            avatarCursor:_av?'cursor:pointer;':'',
            onViewAvatar:_av?self.openAvatarLightbox(_av):null,
            dateLabel:l.date?Utils.fmtMed(new Date(l.date+'T00:00:00')):'',
            typeLabel:l.type==='mc'?'MC':'Personal Leave',
            typeBg:l.type==='mc'?'#fdf6e9':'#fefce8',
            typeColor:l.type==='mc'?'#b9791a':'#7c5e00',
            typeBorder:l.type==='mc'?'#f0e2c2':'#fde68a',
            statusLabel:l.status==='approved'?'Approved':'Declined',
            statusColor:l.status==='approved'?'#1f8a5b':'#c0392b',
            statusBg:l.status==='approved'?'#e7f3ec':'#f7e4e1',
            statusBorder:l.status==='approved'?'#b8dfc9':'#f1d0cc',
            leftBar:l.status==='approved'?'#1f8a5b':'#c0392b',
            reason:l.reason||'',
            hasReason:!!(l.reason),
            reviewedBy:l.reviewed_by||'',
            rejectionReason:l.rejection_reason||'',
            hasRejectionReason:!!(l.rejection_reason&&l.status==='rejected'),
          });}),
          hasProcessedLeaves:(s.processedLeaves||[]).length>0,
          processedLeavesHidden:!!(s.processedLeavesHidden),
          toggleProcessedLeaves:()=>self.setState({processedLeavesHidden:!s.processedLeavesHidden}),
          processedLeavesSearch:s.processedLeavesSearch||'',
          onProcessedLeavesSearch:(e)=>self.setState({processedLeavesSearch:e.target.value,processedLeavesPage:1}),
          clearProcessedLeavesSearch:()=>self.setState({processedLeavesSearch:'',processedLeavesPage:1}),
          hasProcessedLeavesSearch:!!_plq,
          setPlFilterAll:()=>self.setState({processedLeavesFilter:'all',processedLeavesPage:1}),
          setPlFilterMc:()=>self.setState({processedLeavesFilter:'mc',processedLeavesPage:1}),
          setPlFilterPersonal:()=>self.setState({processedLeavesFilter:'personal',processedLeavesPage:1}),
          plFilterAllStyle:_plPill(_plf==='all'),
          plFilterMcStyle:_plPill(_plf==='mc'),
          plFilterPersonalStyle:_plPill(_plf==='personal'),
          processedLeavesFilterNoResults:(s.processedLeaves||[]).length>0&&_plTotal===0,
          processedLeavesShowPagination:_plTotalPages>1,
          processedLeavesPrevOpacity:_plPage>1?'1':'0.35',
          processedLeavesNextOpacity:_plPage<_plTotalPages?'1':'0.35',
          processedLeavesPageLabel:`${_plPage} / ${_plTotalPages}`,
          processedLeavesPagePrev:()=>self.setState({processedLeavesPage:Math.max(1,_plPage-1)}),
          processedLeavesPageNext:()=>self.setState({processedLeavesPage:Math.min(_plTotalPages,_plPage+1)}),
        };
      })(),
      ...
      adminsList:(s.adminsList||[]).map(a=>{
        ...
        return {
          ...
          roleBadgeBg:a.role==='superadmin'?'#fdf6e9':'#f0f2f5',
          roleBadgeColor:a.role==='superadmin'?'#b9791a':'#6b7688',
          avatarRing:a.role==='superadmin'?'box-shadow:0 0 0 2.5px #e8c77a;':'',
