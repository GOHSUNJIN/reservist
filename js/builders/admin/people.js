      ...
      ...(()=>{
        ...
        return {
          pendingLeaves:_sorted.map(l=>{const _ms=l.created_at?_nowMs-new Date(l.created_at).getTime():0,_d=Math.floor(_ms/86400000),isExpired=_ms>_2d,timeAgo=!l.created_at?'':Utils.hhmm(new Date(l.created_at)),_av=s.avatars[l.personnel_id]||'';return({
            ...
            isApproveOpen:s.approveLeaveId===l.id,
            approveLeaveNote:s.approveLeaveId===l.id?s.approveLeaveNote:'',
            onApproveLeaveNote:self.onApproveLeaveNote,
            confirmApproveLeave:self.confirmApproveLeave,
            cancelApproveLeave:self.cancelApproveLeave,
            showActions:s.rejectLeaveId!==l.id&&s.approveLeaveId!==l.id,
            timeAgo,isExpired,
            ...
            confirmApprovalBusyOpacity:s.approvingLeaveId===l.id?'0.45':'1',
            confirmApprovalBusyPE:s.approvingLeaveId===l.id?'none':'auto',
          });
          }),
