  ...
  _buildCheckin: function(s, accent, hqName) {
    ...
    const phases=phaseDefs.map(pd=>{
      ...
      return {
        ...
        stepShadow:done?'0 2px 10px rgba(31,138,91,.30)':isActive?(lateActive?'0 2px 10px rgba(185,121,26,.30)':'0 2px 10px rgba(47,95,208,.28)'):'none',
        connectorBg:done?'#1f8a5b':'#e3e6ec',
        activeAccent:(!done&&!locked&&(inWin||pastWin))?'border-left:2.5px solid '+accent+';padding-left:10px;':'',
