  ...
  _buildCheckin: function(s, accent, hqName) {
    ...
    const phases=phaseDefs.map(pd=>{
      ...
      return {
        ...
        done, notDone:!done, notLockedNotDone:!locked&&!done, time:time||'-', doneText,
        ...
        connectorStyle:done?'width:2px;background:#1f8a5b;border-radius:2px;':'width:0;border-left:2px dashed #d0d5e0;',
        rowPadBot:done?'8px':'16px',
        subLabel:win?(pd.key==='p4'?'From '+win[0]:win[0]+' to '+win[1]):'',
        timeWindowPill:win?(pd.key==='p4'?'From '+win[0]:win[0]+' – '+win[1]):'',
        showTimeWindowPill:!done&&!locked&&(inWin||pastWin)&&!!win,
        btnLabel,
